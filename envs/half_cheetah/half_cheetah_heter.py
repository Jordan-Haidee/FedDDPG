...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def generate_heter_config(self):
        ...
        # 后腿, 前腿末足长度
        bthigh_geom = root.find("worldbody/body/body[@name='bthigh']/body/body/geom")
        fthigh_geom = root.find("worldbody/body/body[@name='fthigh']/body/body/geom")
        r1, _ = bthigh_geom.get("size").split(" ")
        r2, _ = fthigh_geom.get("size").split(" ")
        x1, y1, z1 = bthigh_geom.get("pos").split(" ")
        l1 = 0.05 + 0.10 * self.heter[1]
        l2 = 0.05 + 0.10 * self.heter[2]
        bthigh_geom.set("pos", f"{x1} {y1} {-0.053-0.10*self.heter[1]}")
        bthigh_geom.set("size", f"{r1} {l1}")
        fthigh_geom.set("size", f"{r2} {l2}")
